...
| XSD              | XML                | Moderate      | XML well-formedness, matching among global roots, namespace-prefixed local names, XML declarations, recursive named/inline complex types, user-supplied `xs:include`/`xs:import` sources, ordered sequences, choices, all-groups, attributes, prohibited attributes, nillable elements, min/max occurs including unbounded, common built-in scalar types, simpleType enum/pattern/length/numeric facets | No full XSD 1.0/1.1 compliance, automatic local/remote include/import fetching, group refs, nested particles, wildcards, substitution groups, mixed content, derivation, list/union simple types, identity constraints, Schematron, or Relax NG |
