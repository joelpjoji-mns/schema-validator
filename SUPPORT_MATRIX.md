...
| XSD              | XML                | Moderate      | XML well-formedness, matching among global roots, namespace-prefixed local names, XML declarations, recursive named/inline complex types, ordered sequences, choices, all-groups, attributes, prohibited attributes, nillable elements, min/max occurs including unbounded, common built-in scalar types, simpleType enum/pattern/length/numeric facets | No full XSD 1.0/1.1 compliance, include/import loading, group refs, nested particles, wildcards, substitution groups, mixed content, derivation, list/union simple types, identity constraints, Schematron, or Relax NG |
...
The XSD adapter is deliberately a practical XSD-lite validator, not a full standards processor. It catches common complex XML message problems and highlights both XML and schema locations. Features that can change correctness but are not implemented fail closed with unsupported-feature diagnostics. Full XSD support would need a schema-aware WASM validator or an optional backend service while keeping this static GitHub Pages app as the UI.
